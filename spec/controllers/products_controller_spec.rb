...
describe ProductsController, :type => :controller do
  ...
  describe "PATCH #update" do
    before :each do
      @product = create(:product, name: "Camera", price: 799.00)
    end
    ...
    context "with valid attributes" do
      ...
      it "updates the product in the database" do
        patch :update, id: @product, product: attributes_for(:product,
          name: "New Camera", price: 799.00)
        @product.reload
        expect(@product.name).to eq("New Camera")
        expect(@product.price).to eq(799.00)
      end
    ...
    context "with invalid attributes" do
      it "does not update the product" do
        patch :update, id: @product, product: attributes_for(:product,
          name: "New Camera", price: nil)
        @product.reload
        expect(@product.name).not_to eq("New Camera")
        expect(@product.price).to eq(799.00)
      end
